...
const TN0915_VERSION = "tn0915_v02";
const TN0915_ROUTE = "/p/0915.html";
const TN0915_INSTANCE = "__TN0915_V02_INSTANCE__";
...
function buildCharacter(THREE, mat, colors) {
  ...
  // Normalized front proportions: total height ≈ 4.35, belt ≈ 44%, chin ≈ 67%.
  const HIPS = addJoint(root, "HIPS", 0, 1.73, 0);
  const SPINE = addJoint(HIPS, "SPINE", 0, 0.23, 0);
  const CHEST = addJoint(SPINE, "CHEST", 0, 0.31, 0);
  const NECK = addJoint(CHEST, "NECK", 0, 0.42, 0);
  const HEAD = addJoint(NECK, "HEAD", 0, 0.10, 0);
  Object.assign(joints, { HIPS, SPINE, CHEST, NECK, HEAD });

  const torsoGeometry = latheBodyGeometry(THREE, [
    [-0.54, 0.53], [-0.43, 0.57], [-0.12, 0.59], [0.20, 0.56], [0.43, 0.49], [0.54, 0.37],
  ], 16, 0.80);
  ...
  const chestBib = shapeMesh(THREE, [
    [-0.27, 0.35], [-0.13, 0.28], [0, 0.43], [0.13, 0.28], [0.27, 0.35],
    [0.25, -0.29], [0.13, -0.22], [0, -0.39], [-0.13, -0.22], [-0.25, -0.29],
  ], mat.furLight, 0.032);
  chestBib.position.set(0, -0.02, 0.463);
  CHEST.add(chestBib);

  const chestTuft = shapeMesh(THREE, [[-0.07, 0.05], [0, -0.11], [0.07, 0.05], [0, 0.10]], mat.white, 0.024);
  chestTuft.position.set(0, -0.03, 0.489);
  CHEST.add(chestTuft);

  const pelvis = mesh(taperedBoxGeometry(THREE, 1.18, 0.50, 0.68, 0.92), mat.pants, "PELVIS_MESH");
  pelvis.position.y = -0.07;
  HIPS.add(pelvis);

  const belt = mesh(roundedPrismGeometry(THREE, 1.20, 0.70, 0.17, 0.08), mat.belt, "BELT");
  belt.position.set(0, 0.18, 0);
  HIPS.add(belt);
  const buckle = mesh(roundedPrismGeometry(THREE, 0.29, 0.065, 0.23, 0.04), mat.metal, "BELT_BUCKLE");
  buckle.position.set(0, 0.18, 0.370);
  HIPS.add(buckle);
  const buckleInset = mesh(roundedPrismGeometry(THREE, 0.16, 0.025, 0.11, 0.018), mat.belt, "BELT_BUCKLE_INSET");
  buckleInset.position.set(0, 0.18, 0.407);
  ...
  function buildArm(side, sign) {
    const SHOULDER = addJoint(CHEST, `SHOULDER_${side}`, sign * 0.48, 0.32, 0);
    const UPPER = addJoint(SHOULDER, `UPPER_ARM_${side}`, 0, 0, 0);
    const ELBOW = addJoint(UPPER, `ELBOW_${side}`, 0, -0.50, 0);
    const FOREARM = addJoint(ELBOW, `FOREARM_${side}`, 0, 0, 0);
    const WRIST = addJoint(FOREARM, `WRIST_${side}`, 0, -0.40, 0);
    ...
    const upperMesh = mesh(taperedLimbGeometry(THREE, 0.225, 0.195, 0.52, 12), mat.fur, `UPPER_ARM_${side}_MESH`);
    upperMesh.position.y = -0.25;
    UPPER.add(upperMesh);

    const foreMesh = mesh(taperedLimbGeometry(THREE, 0.205, 0.17, 0.42, 12), mat.fur, `FOREARM_${side}_MESH`);
    foreMesh.position.y = -0.20;
    FOREARM.add(foreMesh);

    const cuff = mesh(new THREE.CylinderGeometry(0.19, 0.205, 0.25, 12), mat.glove, `WRIST_${side}_CUFF`);
    cuff.position.y = -0.305;
    FOREARM.add(cuff);

    const fist = mesh(new THREE.SphereGeometry(0.235, 14, 10), mat.glove, `FIST_${side}`);
    fist.scale.set(1.08, 1.05, 0.92);
    fist.position.set(sign * 0.035, -0.11, 0.035);
    HAND.add(fist);

    const thumb = mesh(new THREE.SphereGeometry(0.105, 12, 8), mat.glove, `THUMB_${side}`);
    thumb.scale.set(0.78, 1.05, 0.76);
    thumb.position.set(-sign * 0.15, -0.10, 0.13);
    HAND.add(thumb);

    for (let i = 0; i < 2; i += 1) {
      const groove = mesh(new THREE.BoxGeometry(0.012, 0.085, 0.025), mat.maskDark, `KNUCKLE_GROOVE_${side}_${i}`);
      groove.position.set(sign * (-0.035 + i * 0.075), -0.085, 0.220);
      HAND.add(groove);
    }
  }

  function buildLeg(side, sign) {
    const THIGH = addJoint(HIPS, `THIGH_${side}`, sign * 0.33, -0.02, 0);
    const KNEE = addJoint(THIGH, `KNEE_${side}`, 0, -0.58, 0);
    const SHIN = addJoint(KNEE, `SHIN_${side}`, 0, 0, 0);
    const ANKLE = addJoint(SHIN, `ANKLE_${side}`, 0, -0.61, 0.01);
    ...
    const thighMesh = mesh(taperedLimbGeometry(THREE, 0.37, 0.33, 0.64, 12), mat.pants, `THIGH_${side}_MESH`);
    thighMesh.position.y = -0.29;
    THIGH.add(thighMesh);

    const shinMesh = mesh(taperedLimbGeometry(THREE, 0.33, 0.275, 0.67, 12), mat.pants, `SHIN_${side}_MESH`);
    shinMesh.position.y = -0.305;
    SHIN.add(shinMesh);

    [
      [-0.47, 0.275, 0.060],
      [-0.565, 0.265, 0.055],
    ].forEach(([y, radius, tube], index) => {
      const fold = mesh(new THREE.TorusGeometry(radius, tube, 6, 16), mat.pants, `PANT_FOLD_${side}_${index}`);
      fold.rotation.x = Math.PI / 2;
      fold.position.y = y;
      SHIN.add(fold);
    });

    const sole = mesh(roundedPrismGeometry(THREE, 0.72, 1.02, 0.18, 0.17), mat.sole, `SOLE_${side}`);
    sole.position.set(0, -0.438, 0.18);
    FOOT.add(sole);

    const shoe = mesh(roundedPrismGeometry(THREE, 0.64, 0.86, 0.33, 0.18), mat.cap, `SHOE_${side}`);
    shoe.position.set(0, -0.29, 0.17);
    FOOT.add(shoe);

    const toe = mesh(roundedPrismGeometry(THREE, 0.63, 0.34, 0.22, 0.14), mat.white, `TOE_${side}`);
    toe.position.set(0, -0.25, 0.49);
    FOOT.add(toe);

    for (let i = 0; i < 3; i += 1) {
      const lace = mesh(new THREE.BoxGeometry(0.42, 0.024, 0.040), mat.white, `LACE_${side}_${i}`);
      lace.position.set(0, -0.10 + i * 0.010, 0.12 + i * 0.075);
  ...
  function buildHead() {
    ...
    headMesh.scale.set(1.02, 0.90, 0.84);
    headMesh.position.y = 0.47;
    HEAD.add(headMesh);
    ...
    [1, -1].forEach((sign) => {
      ...
      eye.scale.set(0.92, 1.02, 0.40);
      eye.position.set(sign * 0.23, 0.57, 0.622);
      eye.rotation.z = sign * 0.20;
      HEAD.add(eye);

      const pupil = mesh(new THREE.SphereGeometry(0.064, 14, 9), mat.pupil, `PUPIL_${sign > 0 ? "L" : "R"}`);
      pupil.scale.set(0.68, 1.0, 0.36);
      pupil.position.set(sign * 0.205, 0.555, 0.682);
      pupil.rotation.z = sign * 0.20;
      HEAD.add(pupil);

      const brow = mesh(roundedPrismGeometry(THREE, 0.31, 0.05, 0.075, 0.025), mat.maskDark, `BROW_${sign > 0 ? "L" : "R"}`);
      brow.position.set(sign * 0.235, 0.715, 0.690);
  ...
  function addFaceChevron(sign) {
    const points = [[0.02, 0.24], [0.24, 0.15], [0.15, 0.05], [0.31, -0.02], [0.12, -0.14], [-0.07, -0.03]];
    const chevron = shapeMesh(THREE, points, mat.furLight, 0.028);
    chevron.scale.x = sign;
    chevron.position.set(sign * 0.29, 0.47, 0.588);
  ...
  function addMask(sign) {
    ...
    patch.scale.x = sign;
    patch.position.set(sign * 0.07, 0.57, 0.619);
    patch.rotation.y = sign * 0.16;
    HEAD.add(patch);
  }

  function addEar(sign) {
    const earGeo = extrudedPolygonGeometry(THREE, [[0, -0.22], [0.27 * sign, 0.16], [0.03 * sign, 0.24]], 0.14);
    const ear = mesh(earGeo, mat.skin, `EAR_${sign > 0 ? "L" : "R"}`);
    ear.position.set(sign * 0.58, 0.96, 0.03);
    ear.rotation.z = sign * -0.24;
    HEAD.add(ear);

    const innerGeo = extrudedPolygonGeometry(THREE, [[0, -0.12], [0.15 * sign, 0.11], [0.025 * sign, 0.16]], 0.018);
    const inner = mesh(innerGeo, mat.earInner, `EAR_INNER_${sign > 0 ? "L" : "R"}`);
    inner.position.set(sign * 0.58, 0.97, 0.115);
    inner.rotation.z = sign * -0.24;
    HEAD.add(inner);
  }

  function addCap() {
    const crown = mesh(new THREE.SphereGeometry(0.72, 24, 10, 0, Math.PI * 2, 0, Math.PI / 2), mat.cap, "CAP_CROWN");
    crown.scale.set(1.04, 0.90, 0.92);
    crown.position.set(0, 0.88, -0.01);
    HEAD.add(crown);

    const band = mesh(new THREE.CylinderGeometry(0.69, 0.73, 0.14, 24, 1, true), mat.capSoft, "CAP_BAND");
    band.scale.z = 0.92;
    band.position.set(0, 0.84, 0);
    HEAD.add(band);

    const billShape = new THREE.Shape();
    billShape.moveTo(-0.56, 0.05);
    billShape.quadraticCurveTo(-0.61, -0.32, -0.37, -0.58);
    billShape.quadraticCurveTo(0, -0.72, 0.37, -0.58);
    billShape.quadraticCurveTo(0.61, -0.32, 0.56, 0.05);
    billShape.closePath();
    ...
    bill.position.set(0, 0.78, 0.38);
    bill.rotation.x = 0.035;
    HEAD.add(bill);

    const button = mesh(new THREE.CylinderGeometry(0.034, 0.039, 0.025, 12), mat.cap, "CAP_BUTTON");
    button.position.set(0, 1.535, -0.01);
    HEAD.add(button);
  }
