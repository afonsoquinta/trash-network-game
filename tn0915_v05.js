/*
 * TRASH NETWORK — procedural raccoon player v05
 * ------------------------------------------------
 ...
 *   <div id='tn0915' data-tn-raccoon-game='true'></div>
 *   <script defer='defer'
 *           data-target='#tn0915'
 *           src='https://YOUR-HOST/tn0915_v05.js'></script>
 *
...
(() => {
  ...
  const BUILD = 'tn0915-v05.0.0';
  const LOCK = Symbol.for('trash-network.raccoon.app');
  const tag = document.currentScript || Array.from(document.scripts).find((script) =>
    /(?:^|\/)tn0915_v05\.js(?:[?#]|$)/i.test(script.src || '')
  ) || null;
  const tagData = tag ? { ...tag.dataset } : {};
  const previous = globalThis[LOCK];

  const allowedPath = /\/p\/0915\.html\/?$/i.test(globalThis.location?.pathname || '') ||
    /^(localhost|127\.0\.0\.1)$/i.test(globalThis.location?.hostname || '') ||
    globalThis.TN_RACCOON_ALLOW_ANY_PATH === true || tagData.allowOtherPath === 'true';
  ...
  function findOrCreateHost() {
    let host = null;
    if (tagData.target) {
      try { host = document.querySelector(tagData.target); } catch (_) { /* invalid custom selector */ }
    }
    if (!host) host = document.getElementById('tn0915');
    if (!host) host = document.querySelector('[data-tn-raccoon-game]');
    if (host) return host;
  ...
  function ensureBloggerStage() {
    if (typeof document === 'undefined' || !document.body?.classList.contains('tn-static_page')) return;
    if (document.getElementById('tn0915-layout-fix')) return;
    const style = document.createElement('style');
    style.id = 'tn0915-layout-fix';
    style.textContent = `
      html body.tn-static_page > .content > .content-fauxcolumns { display:none !important; }
      html body.tn-static_page > .content > .content-outer {
    ...
    class TrashRaccoonGame {
      ...
      _createRaccoon() {
        ...
        const materials = {
          ...
          furShade: new THREE.MeshStandardMaterial({ color: 0x68666b, roughness: 0.98, metalness: 0, flatShading: true }),
          ...
          tailBase: new THREE.MeshStandardMaterial({ color: 0x7d797d, roughness: 1, metalness: 0, flatShading: true, transparent: true }),
          tailDark: new THREE.MeshStandardMaterial({ color: 0x3b393d, roughness: 1, metalness: 0, flatShading: true, transparent: true }),
          tailLight: new THREE.MeshStandardMaterial({ color: 0x8d888e, roughness: 1, metalness: 0, flatShading: true, transparent: true }),
        ...
        const rigRoot = bone('root', modelRoot, 0, 0, 0);
        const pelvis = bone('pelvis', rigRoot, 0, 3.05, 0);
        ...
        const jaw = bone('jaw', head, 0, 0.18, 0.93);
        const capBone = bone('cap', head, 0, 0, 0);
        const earL = bone('ear_L', head, -1.08, 2.03, 0.00);
        const earR = bone('ear_R', head, 1.08, 2.03, 0.00);
        const eyeL = bone('eye_L', head, -0.44, 0.90, 0.94);
        const eyeR = bone('eye_R', head, 0.44, 0.90, 0.94);
        const pupilL = bone('pupil_L', eyeL, 0.045, -0.045, 0.185);
        const pupilR = bone('pupil_R', eyeR, -0.045, -0.045, 0.185);
        const browL = bone('brow_L', head, -0.16, 1.10, 1.10);
        const browR = bone('brow_R', head, 0.16, 1.10, 1.10);

        const clavicleL = bone('clavicle_L', chest, -0.24, 0.08, 0);
        const upperArmL = bone('upperArm_L', clavicleL, -0.72, 0.06, 0);
        ...
        const clavicleR = bone('clavicle_R', chest, 0.24, 0.08, 0);
        const upperArmR = bone('upperArm_R', clavicleR, 0.72, 0.06, 0);
        ...
        const hipL = bone('hip_L', pelvis, -0.72, 0, 0);
        ...
        const hipR = bone('hip_R', pelvis, 0.72, 0, 0);
          ...
          for (let layer = 0; layer < 2; layer++) {
            for (let j = 0; j <= rows; j++) {
              ...
              for (let i = 0; i <= cols; i++) {
                const u = i / cols * 2 - 1;
                const x = u * (1.02 + 0.18 * v - 0.06 * v * v);
                const z = 0.24 + 0.76 * v - 0.035 * u * u;
                const y = 0.07 - 0.035 * v + 0.03 * u * u - layer * thickness;
        ...
        const createSkinnedLoft = (rings, radialSegments, skeletonBones, material, name, parent, position) => {
          ...
          positions.push(rings[ringCount - 1].x || 0, rings[ringCount - 1].y, rings[ringCount - 1].z || 0);
          skinIndices.push(skeletonBones.length - 1, 0, 0, 0);
          skinWeights.push(1, 0, 0, 0);
          ...
          for (let s = 0; s < radialSegments; s++) {
            ...
            const last = (ringCount - 1) * radialSegments;
            indices.push(lastCenter, last + s, last + next);
          }
          const geometry = new THREE.BufferGeometry();
          geometry.setAttribute('position', new THREE.Float32BufferAttribute(positions, 3));
        ...
        const torso = createSkinnedLoft([
          { y: 0.02, rx: 0.66, rz: 0.45 },
          { y: 0.34, rx: 0.70, rz: 0.47, twist: 0.03 },
          { y: 0.72, rx: 0.77, rz: 0.49 },
          { y: 1.08, rx: 0.88, rz: 0.52, twist: -0.03 },
          { y: 1.40, rx: 0.98, rz: 0.55 },
          { y: 1.67, rx: 0.91, rz: 0.51 },
          { y: 1.80, rx: 0.66, rz: 0.40 }
        ], 12, [pelvis, spine01, chest], materials.fur, 'body_skinned', modelRoot, [0, 3.05, 0]);

        const pantRings = [
          { y: 0.05, rx: 0.40, rz: 0.33 },
          { y: -0.28, rx: 0.47, rz: 0.36, twist: 0.04 },
          { y: -0.68, rx: 0.50, rz: 0.36 },
          { y: -1.02, rx: 0.49, rz: 0.35, twist: -0.04 },
          { y: -1.36, rx: 0.62, rz: 0.42 },
          { y: -1.72, rx: 0.68, rz: 0.45, twist: 0.05 },
          { y: -2.04, rx: 0.65, rz: 0.43 },
          { y: -2.18, rx: 0.54, rz: 0.37 }
        ];
        const pantsL = createSkinnedLoft(pantRings, 10, [hipL, kneeL, ankleL], materials.pants,
          'pants_leg_L_skinned', modelRoot, [-0.72, 3.05, 0]);
        const pantsR = createSkinnedLoft(pantRings.map((r) => ({ ...r, twist: -(r.twist || 0) })), 10,
          [hipR, kneeR, ankleR], materials.pants, 'pants_leg_R_skinned', modelRoot, [0.72, 3.05, 0]);
        ...
        const makeTail = () => {
          ...
          const skinWeights = [];
          const geometry = new THREE.BufferGeometry();
          const bandMaterials = [materials.tailBase, materials.tailDark, materials.tailLight,
            materials.tailDark, materials.tailLight, materials.tailDark, materials.tailLight,
            materials.tailDark];
          for (let r = 0; r < ringCount; r++) {
            const u = r / (ringCount - 1);
            const radius = 0.25 * (1 - u) + 0.05 * u + 0.34 * Math.pow(Math.sin(Math.PI * u), 0.78);
            ...
            for (let s = 0; s < radial; s++) {
              ...
              positions.push(Math.cos(angle) * radius, -u * length, Math.sin(angle) * radius * 0.72);
              skinIndices.push(a, b, 0, 0);
              skinWeights.push(1 - wb, wb, 0, 0);
            }
          }
          ...
          skinIndices.push(tailBones.length - 1, 0, 0, 0);
          skinWeights.push(1, 0, 0, 0);
          for (let r = 0; r < ringCount - 1; r++) {
            for (let s = 0; s < radial; s++) {
              ...
              indices.push(a, c, b, b, c, d);
              const angleMid = (s + 0.5) / radial * Math.PI * 2;
              const jaggedU = THREE.MathUtils.clamp(
                (r + 0.5) / (ringCount - 1) + Math.cos(angleMid * 3) * 0.026,
                0, 0.999
              );
              geometry.addGroup(start, 6,
                Math.min(bandMaterials.length - 1, Math.floor(jaggedU * bandMaterials.length)));
            }
          }
          let capStart = indices.length;
          for (let s = 0; s < radial; s++) {
            const next = (s + 1) % radial;
            indices.push(rootCenter, next, s);
          }
          geometry.addGroup(capStart, radial * 3, 0);
          capStart = indices.length;
          const lastRing = (ringCount - 1) * radial;
          for (let s = 0; s < radial; s++) {
            const next = (s + 1) % radial;
            indices.push(tipCenter, lastRing + s, lastRing + next);
          }
          geometry.addGroup(capStart, radial * 3, bandMaterials.length - 1);
          geometry.setAttribute('position', new THREE.Float32BufferAttribute(positions, 3));
          geometry.setAttribute('skinIndex', new THREE.Uint16BufferAttribute(skinIndices, 4));
          geometry.setAttribute('skinWeight', new THREE.Float32BufferAttribute(skinWeights, 4));
          geometry.setIndex(indices);
        ...
        addMesh(pelvis, new THREE.SphereGeometry(1, 12, 8), materials.pants, 'pants_hips',
          [0, 0.04, 0], [1.00, 0.43, 0.59]);
        addMesh(pelvis, new THREE.CylinderGeometry(0.94, 0.98, 0.23, 12), materials.cap, 'belt',
          [0, 0.23, 0], [1, 1, 0.72]);
        ...
        pelvis.add(buckle);
        addMesh(buckle, new THREE.BoxGeometry(0.58, 0.10, 0.055), materials.metal, 'buckle_top', [0, 0.20, 0]);
        addMesh(buckle, new THREE.BoxGeometry(0.58, 0.10, 0.055), materials.metal, 'buckle_bottom', [0, -0.20, 0]);
        addMesh(buckle, new THREE.BoxGeometry(0.10, 0.40, 0.055), materials.metal, 'buckle_left', [-0.24, 0, 0]);
        addMesh(buckle, new THREE.BoxGeometry(0.10, 0.40, 0.055), materials.metal, 'buckle_right', [0.24, 0, 0]);
        ...
        const backFurUpper = makePanel([
          [-0.78, 0.17], [0.78, 0.17], [0.66, 0.00], [0.48, 0.06],
          [0.31, -0.14], [0, -0.02], [-0.31, -0.14], [-0.48, 0.06], [-0.66, 0.00]
        ], 0.042, 0);
        addMesh(chest, backFurUpper, materials.furShade, 'back_fur_upper',
          [0, -0.22, -0.53], [1, 1, 1], [0, Math.PI, 0]);
        const backFurMiddle = makePanel([
          [-0.72, 0.16], [0.72, 0.16], [0.58, -0.01], [0.40, 0.05],
          [0.22, -0.16], [0, -0.04], [-0.22, -0.16], [-0.40, 0.05], [-0.58, -0.01]
        ], 0.04, 0);
        addMesh(chest, backFurMiddle, materials.fur, 'back_fur_middle',
          [0, -0.61, -0.52], [1, 1, 1], [0, Math.PI, 0]);
        const backFurLower = makePanel([
          [-0.60, 0.14], [0.60, 0.14], [0.46, -0.02], [0.28, 0.03],
          [0, -0.20], [-0.28, 0.03], [-0.46, -0.02]
        ], 0.038, 0);
        addMesh(spine01, backFurLower, materials.furShade, 'back_fur_lower',
          [0, -0.16, -0.49], [1, 1, 1], [0, Math.PI, 0]);
        ...
        const makeArm = (side, upper, elbow, wrist, handBone) => {
          ...
          addMesh(handBone, new THREE.IcosahedronGeometry(0.35, 1), materials.mask,
            `fist_${side}`, [0, -0.25, 0.05], [1.03, 0.94, 0.78]);
        ...
        const headMesh = addMesh(head, new THREE.IcosahedronGeometry(1, 3), materials.fur, 'head_base',
          [0, 0.92, 0], [1.44, 1.12, 0.92]);
        headMesh.geometry.computeVertexNormals();

        const headBackUpper = makePanel([
          [-1.14, 0.20], [1.14, 0.20], [1.02, 0.03], [0.82, 0.09],
          [0.66, -0.11], [0.42, -0.02], [0.20, -0.21], [0, -0.08],
          [-0.20, -0.21], [-0.42, -0.02], [-0.66, -0.11], [-0.82, 0.09],
          [-1.02, 0.03]
        ], 0.05, 0);
        addMesh(head, headBackUpper, materials.furShade, 'head_back_upper',
          [0, 0.64, -0.78], [1, 1, 1], [0, Math.PI, 0]);
        const headBackMiddle = makePanel([
          [-1.02, 0.18], [1.02, 0.18], [0.88, 0.00], [0.68, 0.07],
          [0.51, -0.13], [0.27, -0.04], [0, -0.24], [-0.27, -0.04],
          [-0.51, -0.13], [-0.68, 0.07], [-0.88, 0.00]
        ], 0.048, 0);
        addMesh(head, headBackMiddle, materials.fur, 'head_back_middle',
          [0, 0.35, -0.80], [1, 1, 1], [0, Math.PI, 0]);
        const headBackLower = makePanel([
          [-0.80, 0.16], [0.80, 0.16], [0.66, -0.02], [0.43, 0.04],
          [0.24, -0.17], [0, -0.30], [-0.24, -0.17], [-0.43, 0.04],
          [-0.66, -0.02]
        ], 0.046, 0);
        addMesh(head, headBackLower, materials.furShade, 'head_back_lower',
          [0, 0.08, -0.82], [1, 1, 1], [0, Math.PI, 0]);
        ...
        for (const side of [-1, 1]) {
          ...
          cheekGroup.position.set(side * 0.71, 0.73, 0.78);
          cheekGroup.scale.x = side;
          head.add(cheekGroup);
          addMesh(cheekGroup, makePanel(cheekOuterPoints, 0.035, 0.008), materials.furLight, 'cheek_outer');
          addMesh(cheekGroup, makePanel(cheekInnerPoints, 0.038, 0.008), materials.ivory, 'cheek_ivory', [0, -0.01, 0.030]);
        }
        ...
        addMesh(head, makeEllipsoidPatch(leftMaskPoints, 1.44, 1.12, 0.92, 0.032), materials.mask,
          'face_mask_L', [0, 0.92, 0.092]);
        addMesh(head, makeEllipsoidPatch(rightMaskPoints, 1.44, 1.12, 0.92, 0.032), materials.mask,
          'face_mask_R', [0, 0.92, 0.092]);

        const bridgePoints = [[-0.28, 0.50], [0, 0.72], [0.28, 0.50], [0.18, -0.14], [0, -0.32], [-0.18, -0.14]];
        addMesh(head, makeEllipsoidPatch(bridgePoints, 1.44, 1.12, 0.92, 0.045), materials.maskSoft,
          'mask_bridge', [0, 0.92, 0.102]);

        const eyeWhiteGeometry = new THREE.SphereGeometry(1, 14, 9);
        const eyeLMesh = addMesh(eyeL, eyeWhiteGeometry, materials.eye, 'eye_white_L',
          [0, -0.02, 0], [0.30, 0.27, 0.105], [0, 0, -0.16]);
        const eyeRMesh = addMesh(eyeR, eyeWhiteGeometry, materials.eye, 'eye_white_R',
          [0, -0.02, 0], [0.30, 0.27, 0.105], [0, 0, 0.16]);
        addMesh(pupilL, new THREE.SphereGeometry(1, 12, 8), materials.iris, 'iris_L', [0, 0, 0], [0.125, 0.145, 0.052]);
        addMesh(pupilR, new THREE.SphereGeometry(1, 12, 8), materials.iris, 'iris_R', [0, 0, 0], [0.125, 0.145, 0.052]);
        addMesh(pupilL, new THREE.SphereGeometry(1, 10, 7), materials.pupil, 'pupil_black_L', [0, 0, 0.042], [0.070, 0.090, 0.032]);
        addMesh(pupilR, new THREE.SphereGeometry(1, 10, 7), materials.pupil, 'pupil_black_R', [0, 0, 0.042], [0.070, 0.090, 0.032]);
        addMesh(pupilL, new THREE.SphereGeometry(1, 7, 5), materials.eye, 'eye_glint_L', [-0.030, 0.040, 0.072], [0.022, 0.028, 0.012]);
        addMesh(pupilR, new THREE.SphereGeometry(1, 7, 5), materials.eye, 'eye_glint_R', [-0.030, 0.040, 0.072], [0.022, 0.028, 0.012]);

        const lidGeometry = makePanel([
          [-0.34, 0.10], [0.34, 0.01], [0.31, -0.11], [-0.31, -0.035]
        ], 0.045, 0.006);
        addMesh(head, lidGeometry, materials.mask, 'upper_lid_L', [-0.44, 1.07, 1.085], [0.94, 0.90, 1], [0, 0, -0.18]);
        addMesh(head, lidGeometry, materials.mask, 'upper_lid_R', [0.44, 1.07, 1.085], [-0.94, 0.90, 1], [0, 0, -0.18]);

        const browGeometry = makePanel([
          [-0.78, 0.03], [-0.68, 0.28], [-0.18, 0.18], [0.10, 0.035],
          [-0.03, -0.10], [-0.49, -0.05]
        ], 0.038, 0.008);
        addMesh(browL, browGeometry, materials.ivory, 'brow_fur_L', [0, 0, 0], [0.80, 0.80, 0.85], [0, 0, -0.22]);
        const browRight = addMesh(browR, browGeometry, materials.ivory, 'brow_fur_R', [0, 0, 0], [-0.80, 0.80, 0.85], [0, 0, 0.22]);
        browRight.material.side = THREE.DoubleSide;

        addMesh(head, new THREE.IcosahedronGeometry(1, 2), materials.furLight, 'muzzle_bridge',
          [0, 0.47, 0.94], [0.29, 0.26, 0.20]);
        addMesh(head, new THREE.IcosahedronGeometry(1, 2), materials.ivory, 'muzzle_pad_L',
          [-0.24, 0.31, 0.97], [0.37, 0.30, 0.25]);
        ...
        const earOuter = makePanel([[-0.40, -0.34], [-0.27, 0.28], [0, 0.64], [0.34, -0.27], [0, -0.46]], 0.19, 0.018);
        const earInner = makePanel([[-0.25, -0.23], [-0.16, 0.18], [0, 0.42], [0.21, -0.18], [0, -0.31]], 0.10, 0.012);
        addMesh(earL, earOuter, materials.fur, 'ear_outer_L', [0, -0.03, -0.03], [0.76, 0.76, 0.76], [0.04, 0.12, 0.14]);
        addMesh(earL, earInner, materials.ivory, 'ear_inner_L', [0, -0.02, 0.12], [0.60, 0.60, 0.60], [0.04, 0.12, 0.14]);
        addMesh(earR, earOuter, materials.fur, 'ear_outer_R', [0, -0.03, -0.03], [-0.76, 0.76, 0.76], [0.04, -0.12, -0.14]);
        addMesh(earR, earInner, materials.ivory, 'ear_inner_R', [0, -0.02, 0.12], [-0.60, 0.60, 0.60], [0.04, -0.12, -0.14]);

        addMesh(capBone, new THREE.SphereGeometry(1, 20, 8, 0, Math.PI * 2, 0, Math.PI / 2), materials.cap,
          'cap_crown', [0, 1.30, -0.05], [1.34, 1.50, 1.04]);
        addMesh(capBone, new THREE.CylinderGeometry(1, 1, 0.22, 20), materials.cap,
          'cap_lower_band', [0, 1.22, -0.05], [1.32, 1, 1.02]);
        addMesh(capBone, makeBrimGeometry(), materials.cap, 'cap_brim', [0, 1.05, 0.19]);
        addMesh(capBone, new THREE.CylinderGeometry(0.09, 0.095, 0.08, 10), materials.capSeam,
          'cap_button', [0, 2.84, -0.05]);
        const seamCurves = [
          [[0, 2.80, -0.04], [-0.68, 2.22, 0.42], [-1.04, 1.34, 0.17]],
          [[0, 2.80, -0.04], [0.68, 2.22, 0.42], [1.04, 1.34, 0.17]],
          [[0, 2.80, -0.05], [0, 2.18, 0.88], [0, 1.31, 1.03]]
        ];
      ...
      _updatePose(dt, travelled, running, moveAmount) {
        ...
        add('upperArm_L', 0.015 * Math.sin(idle), 0, -0.18);
        add('upperArm_R', -0.015 * Math.sin(idle), 0, 0.18);
        ...
        add('tail_00', 0.18, 0, 0.06);
        add('tail_01', 0.045, 0, 0);
        add('tail_02', 0.040, 0, 0);
        add('tail_03', 0.035, 0, 0);
        add('tail_04', 0.030, 0, 0);
        add('tail_05', 0.025, 0, 0);
      ...
      _updateViewDependentVisibility() {
        const tail = this.character?.skins?.tail;
        if (!tail || !this.camera) return;
        const THREE = this.THREE;
        const actor = this.character.actorRoot;
        this._tailCameraVector ||= new THREE.Vector3();
        this._tailForward ||= new THREE.Vector3();
        this._tailCameraVector.copy(this.camera.position).sub(actor.position).setY(0);
        if (this._tailCameraVector.lengthSq() < 1e-8) return;
        this._tailCameraVector.normalize();
        this._tailForward.set(Math.sin(actor.rotation.y), 0, Math.cos(actor.rotation.y));
        const frontDot = this._tailCameraVector.dot(this._tailForward);
        const opacity = 1 - THREE.MathUtils.smoothstep(frontDot, 0.76, 0.94);
        tail.visible = opacity > 0.015;
        const tailMaterials = Array.isArray(tail.material) ? tail.material : [tail.material];
        tailMaterials.forEach((material) => { material.opacity = opacity; });
      }
      ...
      _render(dt) {
        ...
        this._updateViewDependentVisibility();
      ...
      resume(reason = 'manual') {
        ...
        const loop = () => {
          ...
          try {
            ...
            this.raf = 0;
            this.pauseReasons.add('runtime-error');
            this._showStatus(`Erro durante a animação: ${error.message}`);
            console.error('[Trash Network raccoon animation]', error);
          }
        };
        this.raf = requestAnimationFrame(loop);
      }
